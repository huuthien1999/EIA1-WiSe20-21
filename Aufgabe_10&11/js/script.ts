...
var undoneDOMElement: HTMLElement;
let schlüssel: boolean = false;
...
function drawListToDOM(): void {
    ...
    for (let index: number = 0; index < test.length; index++) {
         ...
         */
        todo.innerHTML =  "<span class='check " + test[index].todosChecked + "'><i class='fas fa-check'></i></span>"
                            + test[index].todosText +
...
function updateCounter(): void {
    ...
    for (let index: number = 0; index < test.length; index++) {
        if (test[index].todosChecked == true) {
            done++;
            console.log(done);
        }
        else {
